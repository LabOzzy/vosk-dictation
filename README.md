...
- ❗ GTK3 + PyGObject и все `.typelib` файлы GObject Introspection  
- ❗ Все библиотеки `libgio`, `libglib`, `libgobject`, `libffi`, `libpcre`, `libz`, `libmount`, `libselinux`, `libblkid`  

Полностью автономен — не зависит от системных библиотек. Работает в любой системе, где поддерживается AppImage.
...
📝 История изменений
v1.1 — Расширение совместимости
✅ Ручное добавление всех GTK3 и GObject зависимостей для полной автономности

✅ Полная упаковка libgobject, libgio, libglib, .typelib, gi и пр.

✅ Проверка через ldd, сборка повторно через appimagetool

🧪 AppImage запускается без установленных системных библиотек
